...
By default, any data created inside a container is lost when that container is removed. **Volumes** are Docker's preferred mechanism for persisting data generated by and used by Docker containers.

There are three main types of mounts/volumes in Docker:

### 1. Named Volumes
These are fully managed by Docker and stored in a part of the host filesystem which is managed by Docker (e.g., `/var/lib/docker/volumes/` on Linux). You don't need to know or care about the exact path on the host machine. This is the **best choice for databases** and persistent application data.

**Example (docker run):**
```bash
# Creates a volume named "my-db-data" and mounts it to /data/db in the container
docker run -v my-db-data:/data/db mongo
```

### 2. Bind Mounts
Bind mounts map a specific, exact path on your host machine to a specific path in the container. They are highly dependent on the host machine's directory structure. This is the **best choice for local development** (hot-reloading) where you want the container to instantly see code changes you make in your IDE.

**Example (docker run):**
```bash
# Maps the current host directory ($PWD) to /app in the container
docker run -v $PWD:/app node:18
```

### 3. Anonymous Volumes
These are similar to named volumes, but they don't have a specific human-readable name assigned to them. Docker gives them a random hash name. They are often used in Compose files to *prevent* a bind mount from overriding a specific folder inside the container (like protecting container-installed `node_modules` from your host OS).

**Example (docker-compose.yml):**
```yaml
volumes:
  - .:/app             # ⬅️ Bind mount (maps your host code to the container)
  - /app/node_modules  # ⬅️ Anonymous volume (protects the container's node_modules)
```
